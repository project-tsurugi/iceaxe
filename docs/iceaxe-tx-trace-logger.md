...
（これらのシステムプロパティーの処理はTsurugiConnectorで行われる）
...
SQL実行結果（TsurugiResultSet・TsurugiResultCount）からは以下のタイミングでログを出力する。

- ResultSetからの読み込み時
- ResultSetからの読み込みエラー発生時
- SQL実行の終了時
  - ResultSetの場合、それ以上読み込むレコードが無くなった時や、読むのを途中で止めた時
  - ResultCountの場合、結果チェックを実施した時
- SQL実行結果のクローズ時
...
（トランザクションの生成やコミット・ロールバックの実行等は、「トランザクションのイベント」としてログが出力されるので、トランザクションマネージャーから出力する必要は無い。また、トランザクションはトランザクションマネージャーを使わなくても生成できるので、トランザクションマネージャーが無くても問題ないようにしている）
...
しかしアプリケーション側で例外が発生してトランザクションがロールバックされる場合は、例外が発生したことがログに出力されないと、なぜロールバックされたのかが分からない。（無論、アプリケーションがトランザクションマネージャーを使っていない場合は、例外のログは出せない））
...
executeXxxの後ろの時間は、executeXxxメソッドの実行時間。
...
ただし、全く同じ例外を複数回出力する場合は、2回目以降は例外クラスと例外メッセージだけ出力される。

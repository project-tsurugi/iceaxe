...
クライアント（Iceaxe）とTsurugi DBの間でLOBデータを受け渡す方法はいくつかあり、どの方法を使うかはクライアントアプリケーション（Iceaxeを利用するアプリケーション）側で指定することができますが、データを受け渡すAPI（コーディング方法）は同一です。
...
すなわち、LOBデータをDBに登録する際は、ユーザー（クライアントアプリケーション）がLOBデータのファイルを用意し、Iceaxe経由でそのファイルのパスをTsurugi DBに渡します（Tsurugi DBは渡されたパスのファイルをコピーします）。  
...
IceaxeにおけるCLOBの操作方法はBLOBと同等ですので、CLOBについては上記の点を読み替えてください。
...
4. TsurugiSqlPreparedStatementやTsurugiSqlPreparedQueryを使ったSQL実行メソッドや実行計画取得メソッドの引数にTgBindParameterを渡す。
   - SQL実行メソッドの中でTgRemoteBlobはクローズされる。
   - SQL実行メソッドで使われなかった場合、TgRemoteBlobはTsurugiSessionクローズ時にクローズされる。

TsurugiLobFactoryのアップロードメソッドは、利便性のために、BLOBファイルのPathやInputStream, byte[]を引数で受け取るメソッドが用意されています。

### 特権モードの注意

特権モードで引数がBLOBファイルのPathのアップロードメソッドを使う場合、そのファイルはSQL実行時点でも存在している必要があります。（実際に読まれるのはSQL実行時であるため）
...
>
> デフォルトでは、一時ファイルが作られる場所はシステムプロパティー `java.io.tmpdir` で示されるディレクトリーの下です。
...
void insert(TsurugiTransactionManager tm) throws IOException, InterruptedException {
    ...
    try (var ps = tm.getSession().createStatement(sql, parameterMapping)) {
        ...
        try (var stream = Files.list(Path.of("dir"))) { // アップロード対象ファイル一覧を取得
...
Iceaxe 1.9.0で、特権モードでLOBファイルを扱う際にクライアント側のパスとサーバー側のパスを変換するパスマッピング機能が導入されました。
...
BLOB中継サービスの接続先URI（エンドポイント）は、Tsurugi DBから送られ、Iceaxe（Tsubkauro/Java）内部で使用しています。
